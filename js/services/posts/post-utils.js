...
export function extractImageFromAny(post){
    const images = [];
    //etraiamo tutte le immagini dal post e ritorniamo l'array
    if (!post || !post.body) return null;
    try {
        // Try to find Markdown image
        const markdownMatch = post.body.match(/!\[.*?\]\((.*?)\)/g);
        if (markdownMatch) {
            markdownMatch.forEach((element) => {
                images.push(element);
            });
        }
        // Try to find HTML image
        const htmlMatch = post.body.match(/<img[^>]+src="([^">]+)"/g);
        if (htmlMatch) {
            htmlMatch.forEach((element) => {
                images.push(element);
            });
        }
        // Try to find href with image link ,but without <a>
        const htmlMatch2 = post.body.match(/href="([^"]+\.(?:jpg|png|jpeg|gif|webp)[^"]*)"[^>]*>/ig);       
        if (htmlMatch2) {
            htmlMatch2.forEach((element) => {
                images.push(element);
            });
        }
        // Try to find raw URL or URLs with different patterns or query parameters
        const urlMatch = post.body.match(/(https?:\/\/[^\s<>"']*\.(?:jpg|png|jpeg|gif|webp)(\?[^\s<>"']*)?)/ig);
        if (urlMatch) {
