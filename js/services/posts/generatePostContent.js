...
function normalizeMarkdownLinks(content) {
    // Normalizza i link inline standard
    content = content.replace(
        /\[([^\]]+)\]\(([^)]+)\)/g,
        (match, text, url) => {
            // Rimuovi eventuali spazi all'inizio e alla fine dell'URL
            url = url.trim();
            
            // Gestisci URL relativi o malformati
            if (!url.startsWith('http')) {
                url = 'https://' + url;
            }
            
            // Normalizza i link di Steemit
            if (url.includes('steemit.com')) {
                return transformSteemitLinks(`[${text}](${url})`);
            }
    ...
    // Normalizza i link reference-style
    content = content.replace(
        /\[([^\]]+)\]\[([^\]]*)\]/g,
        (match, text, ref) => {
            // Se il riferimento è vuoto, usa il testo come riferimento
            const reference = ref || text;
            return `[${text}][${reference}]`;
        }
    );

    // Normalizza le definizioni dei link
    content = content.replace(
        /^\s*\[([^\]]+)\]:\s*(\S+)(?:\s+"([^"]+)")?\s*$/gm,
        (match, id, url, title) => {
            url = url.trim();
...
export function generatePostContent(htmlContent) {
    // Normalizza i link prima di qualsiasi altra trasformazione
    htmlContent = normalizeMarkdownLinks(htmlContent);
